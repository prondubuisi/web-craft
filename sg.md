...
`docs/sg9.md` is a data-model maintainability note — internal architecture, not a feature. §3 shipped: `parseJsonColumn` reads tags and includes; corrupt `blocks_json` still throws. §2 (shared Zine field descriptor) and §1 (table-driven `assertBlock` → harvest → applyBag → createBlock) stay plan only. No schema library — `sg3.md` already ruled that out.

`docs/sg10.md` is an async-collaboration polish note — remix, exquisite corpse chains, compilations, explicitly not live/real-time multiplayer (the app stays client-first, server-optional; no live-sync infra exists or is proposed). §1 shipped: the chain contributor now sees `page N` — `turn` was already computed server-side and thrown away client-side. §3 shipped: the compilation picker says "8 of N public issues shown" when it truncates. §2 (remix credit dead-ends when the source isn't loaded locally) stays flagged, not fixed — the honest link needs the source's visibility/share key first. Chain contributions staying sticker-only either way is noted, not proposed as a change.
