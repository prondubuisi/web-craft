...
`docs/sg6.md` is a visual QA pass, read from running screenshots rather than code alone. Item 1 shipped (`--ghost-ink` / `--ghost-border` on `.zine-page` and `.modal`). Still open: (2) Board's filter row and post-kind picker share one class; (3) Profile duplicates the vibe when there's no pen name; (4) Help's glossary cards force a 180px minimum height; (5) Stream's lane/vibe/sort/tag filters need visual grouping, needs a design call first. One open tone question (seeded first-visit mail) is not a fix.
