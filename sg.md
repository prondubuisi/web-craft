...
`docs/ARCHITECTURE_PLAN.md` Round 2 is that note. **§8–12 done:** Preview uses `useRemote`; remote+local merges use `useRemoteWithFallback`; editor undo/redo lives in `useHistory`; `useIssueSocial` has tests; migration ignore is per-file. Remaining: items 13–14.
